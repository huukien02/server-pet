...
router.get('/', function (req, res, next) {
  res.json("Server is Runninggggggggg")
});
...
router.get('/delete/order/:id', function (req, res, next) {
  db3.deleteOne({ _id: req.params.id }, (err) => {
    if (err) throw err;
    res.json("Xác nhận hàng thành công")
  })
})
